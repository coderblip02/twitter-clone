      ...
      <div className="">
        {Object.values(providers).map((provider)=>(
            <div className=" flex flex-col items-center">
                ...
                <p className="text-center text-sm italic my-10">Twitter Clone Created for fun</p>
                <button onClick={()=>signIn(provider.id, {callbackUrl: "/"})} className="bg-blue-400 text-white rounded-lg p-3 hover:bg-blue-500">Sign In with {provider.name}</button>  
            </div>
        ))}
      </div>
    </div>
